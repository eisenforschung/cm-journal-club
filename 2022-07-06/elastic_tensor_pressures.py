...
class ElasticTensorPressures(AtomisticParallelMaster):
    """
    ...
    def __init__(self, project, job_name):
        ...
        self.input["pressure_path"] = (
            "pressures",
            "HDF path in the reference job to get the pressures from; data will be accessed as ref['output/generic/{pressure_path}']"
        )
    ...
    def collect_output(self):
        ...
        value_paths = {
            ...
            "pressures": self.input["pressure_path"],
            "volume": "volume"
        }
        if self.ref_job.server.run_mode.interactive:
            ...
            for key, path in value_paths.items():
                if key in job["output/generic"].list_nodes():
                    output_data[key] = job[f"output/generic/{path}"]
        else:
            ...
            for key, path in value_paths.items():
                if all(key in job["output/generic"].list_nodes() for job in job_list):
                    output_data[key] = np.array(
                        [job[f"output/generic/{path}"][-1] for job in job_list]
                    )
